...
class GeneticAlgorithm:
    ...
    def create_visualization_window(self):
        ...
        # Кнопки навигации
        self.prev_btn = ttk.Button(nav_frame, text="← Предыдущее", command=self.show_prev_generation)
        self.prev_btn.pack(side=tk.LEFT, padx=5)
        
        self.next_btn = ttk.Button(nav_frame, text="Следующее →", command=self.show_next_generation)
        ...
        self.generation_label = ttk.Label(info_frame, text="0", font=('Arial', 10, 'bold'))
        self.generation_label.pack(side=tk.LEFT, padx=5)
        
        ttk.Label(info_frame, text="Целевая функция:").pack(side=tk.LEFT)
        self.fitness_label = ttk.Label(info_frame, text="0.0", font=('Arial', 10, 'bold'))
    ...
    def update_navigation_buttons(self):
        ...
        self.prev_btn.config(state=tk.NORMAL if self.current_generation_idx > 0 else tk.DISABLED)
        self.next_btn.config(state=tk.NORMAL if self.current_generation_idx < len(self.generations_data) - 1 else tk.DISABLED)
    ...
    def display_generation(self, gen_idx):
        ...
        if 0 <= gen_idx < len(self.generations_data):
            ...
            for idx, (x, y, rot) in enumerate(placements):
                ...
                if x + width > self.config["board_width"] or y + height > self.config["board_height"]:
                    out_of_bounds.append(f"Компонент {idx+1} ({width}x{height}) выходит за границы! [X={x}-{x+width}, Y={y}-{y+height}]")
                    continue
    ...
    def run(self, log=None, visualization_steps=None):
        ...
        vis_steps = visualization_steps if visualization_steps is not None else self.config.get("visualization_steps", [])
        self.generations_data = []
        ...
        for gen in range(1, self.config["generations"] + 1):
            ...
            if gen in vis_steps or gen == self.config["generations"]:
                best_ind = tools.selBest(population, k=1)[0]
                self.generations_data.append((best_ind, gen, best_ind.fitness.values[0]))
